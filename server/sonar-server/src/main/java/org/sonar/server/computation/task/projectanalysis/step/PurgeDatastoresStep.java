...
public class PurgeDatastoresStep implements ComputationStep {
  ...
  private void execute(Component root) {
    try (DbSession dbSession = dbClient.openSession(true)) {
      IdUuidPair rootId = new IdUuidPair(dbIdsRepository.getComponentId(root), root.getUuid());
      projectCleaner.purge(dbSession, rootId, configRepository.getConfiguration(), disabledComponentsHolder.getUuids());
      dbSession.commit();
    }
  }
